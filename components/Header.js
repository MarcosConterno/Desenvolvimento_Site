...
export default function Header() {
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-3">
          <Link href="/">
            <div className="flex items-center justify-center rounded cursor-pointer">
              <img
                src="/img/novas/LogoHorizontalBranca.svg"
              ...
              <div
                className={`relative pb-1 cursor-pointer transition-all duration-200 text-lg ${
                  router.pathname === path
                    ? "font-bold border-b-2 border-[#a07e28]"
                    : "hover:border-b-2 hover:border-[#a07e28]"
                }`}
              >
                {name}
              </div>
              ...
              className={`w-full px-6 py-3 mt-0 rounded-full text-lg md:text-xl font-semibold font-livvic transition-colors duration-300 border-2 animate-pulse
                ${
                  isAssociese
                    ...
                    : "text-white border-white hover:bg-[#d7b661] hover:text-[#324530]"
                }`}
            >
              Associe-se
            </button>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-[#254337]/95 backdrop-blur-sm text-white px-6 py-4 space-y-4 transition-all duration-300">
          {navItems.map(({ name, path }) => (
            <Link key={path} href={path} aria-current={router.pathname === path ? "page" : undefined}>
              <div
